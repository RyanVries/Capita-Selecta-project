...
data_dir_pretrain=r"PretrainingData"
...
pretrain_data,pretrain_labels=get_pretrain_data(data_dir_pretrain,new_shape=sample_shape)
#slice_id = 40
...
if not os.path.exists(results_dir):
    os.mkdir(results_dir)
elif len(os.listdir(results_dir)) != 0 and exp_name!='test':
...
for i,(train_index, val_index) in enumerate(kf.split(pretrain_data)):
    ...
    model = unet_model_3d(input_shape=input_shape,initial_learning_rate=lr,depth=depth,n_base_filters=channels)#batch_normalization=use_batchnorm)
