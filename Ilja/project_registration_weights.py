...
def ltransform_weights(metric,a,b):
    weight = metric*a+b
    return weight
...
def leave_one_out(img_names,data_dir,param_array,result_file,all_results_file,b0,b1):
    ...
    for pat_idx, pat in enumerate(img_names): #over patient
        ...
        weights = ltransform_weights(rel_nmis,b1,b0)
        
        prostate_estm[:,:,:,pat_idx] = atlas_segmentation(prostates_estm_pat,weights=None)
...
leave_one_out(all_image_names,data_dir,param_array,all_results_file,result_file,b0,b1)
